...
const CHAT_MARKDOWN_COMPONENTS = {
  ...
  pre: function MarkdownPre({ node, children, ...props }) {
    ...
    // A ```mermaid fence is a picture the agent drew, so render it and keep its source
...
/**
 * Wraps a message in one batched asset request. Without an environment there is nothing
 * to mint against, and a message with no workspace images has nothing to batch, so
 * either case renders the children directly rather than paying for a provider.
 */
function ChatMarkdownImageBatch(props: {
  readonly environmentId: EnvironmentId | null;
  readonly resources: ReadonlyArray<AssetResource>;
  readonly children: ReactNode;
}) {
  if (props.environmentId === null || props.resources.length === 0) {
    return props.children;
  }
  return (
    <AssetUrlBatchProvider environmentId={props.environmentId} resources={props.resources}>
      {props.children}
    </AssetUrlBatchProvider>
  );
}
...
function ChatMarkdown({
  ...
  const { cwd, imageBaseDir, threadRef, environmentId } = componentState;
  ...
  return (
    ...
    >
      <ChatMarkdownRendererContext value={componentState}>
        <ChatMarkdownImageBatch environmentId={environmentId} resources={batchedImageResources}>
          <ReactMarkdown
