...
it.layer(NodeServices.layer)("EnvironmentAuthPolicy.layer", (it) => {
  ...
  it.effect("uses remote-reachable policy for loopback hosts with Tailscale Serve enabled", () =>
    Effect.gen(function* () {
      const policy = yield* EnvironmentAuthPolicy.EnvironmentAuthPolicy;
      const descriptor = yield* policy.getDescriptor();
  ...
  it.effect("uses remote-reachable policy for loopback hosts marked remoteReachable", () =>
    Effect.gen(function* () {
      const policy = yield* EnvironmentAuthPolicy.EnvironmentAuthPolicy;
      const descriptor = yield* policy.getDescriptor();
