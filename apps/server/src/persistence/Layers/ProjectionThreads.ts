...
import { ModelSelection, ThreadLinkedIssue, ThreadLinkedPullRequest } from "@t3tools/contracts";
    ...
    execute: (row) =>
      sql`
        ...
        VALUES (
          ...
          ${row.linkedIssue === undefined || row.linkedIssue === null ? null : JSON.stringify(row.linkedIssue)},
          ${row.latestTurnId},
          ${row.createdAt},
          ${row.updatedAt},
          ${row.archivedAt},
        ...
        DO UPDATE SET
          ...
          linked_issue_json = excluded.linked_issue_json,
    ...
    execute: ({ threadId }) =>
      sql`
        SELECT
          ...
          linked_issue_json AS "linkedIssue",
