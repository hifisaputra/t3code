...
When the worker is idle, its changes are committed and merged, staging acceptance checks have passed, and its local servers/workers are stopped, call assistant_verify_staging with the summary (including verification evidence), review instructions and the relevant configured deployment targetIds. Omit targetIds to check all configured services. T3 independently checks deployment records/commit ancestry or the optional project command. An agent saying "deployed" is not sufficient. On success the worker is archived and the project is free immediately: start the next issue without waiting for human review.
