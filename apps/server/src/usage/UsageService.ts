...
export const make = Effect.gen(function* () {
  ...
  const readThreadStats = Effect.fn("UsageService.readThreadStats")(function* (
    ...
    yield* ensureRates(false).pipe(Effect.catchCause(() => Effect.void));
    ...
    const aggregator = new ThreadUsageAggregator(
      rates,
  ...
  return { readSummary, readThreadStats, refreshRates } as const;
});
